...
def main():
    ...
    for mmsi_key, rig_snap in main_data.items():
        ...
        if reference_pos and valid_coords(reference_pos.get("latitude"), reference_pos.get("longitude")):
            movement_km = haversine_km(reference_pos["latitude"], reference_pos["longitude"], lat, lon)
    ...
    save_json_atomic({"generated_at": now.isoformat(), "rigs": rig_results}, OUTPUT_PATH)
